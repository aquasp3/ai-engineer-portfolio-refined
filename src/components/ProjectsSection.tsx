...
const ProjectsSection = () => {
  return (
    <GlassSection id="projects">
      <h2 className="text-2xl font-bold mb-1 text-[#8BA194]">Projects</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
        {projects.map((project, i) => (
          ...
          >
            <h3 className="text-lg font-semibold text-[#FFF7E2] mb-2">{project.title}</h3>
            <p className="text-sm font-semibold text-[#8BA194] mb-3">Metric: {project.metric}</p>
            <p className="text-xs uppercase tracking-wide text-[#FFF7E2]/75 mb-2">Tech Stack</p>
            <div className="flex flex-wrap gap-1.5 mb-4">
              {project.tech.map((tech) => (
                <span key={tech} className="px-2.5 py-1 text-xs bg-[#8BA194]/15 text-[#FFF7E2] rounded-full font-mono border border-[#8BA194]/35">
                  {tech}
                </span>
              ))}
            </div>
            <p className="text-sm text-[#FFF7E2]/90 mb-5 flex-1 leading-relaxed">{project.impact}</p>
            <div className="flex flex-col sm:flex-row gap-2">
              <a
                href={project.codeUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex w-full sm:w-auto items-center justify-center gap-2 px-4 h-10 rounded-full text-sm border transition-colors hover:bg-[#8BA194]/80 hover:text-[#4F633D]" style={{ borderColor: "#8BA194", backgroundColor: "rgba(255,247,226,0.05)", color: "#FFF7E2" }}
              >
                <Github size={15} />
                View Code
              </a>
